...
const ResultDisplay: React.FC<ResultDisplayProps> = ({ result, onRetry, onRegenerateText, onTabChange, isLoading }) => {
  if (!result) {
    return (
  ...
  const detectedForbiddenWords = useMemo(() => {
    ...
    if (activeText === '生成を開始しています...') return [];
    return FLATTENED_FORBIDDEN_WORDS.filter(word => activeText.includes(word));
  }, [activeText]);

  const hasViolation = detectedForbiddenWords.length > 0;

  const getHighlightedText = () => {
    if (activeText === '生成を開始しています...') {
      return <p className="text-gray-400 italic">AIが思考を整理しています...</p>;
    }

    if (!hasViolation) {
      return <p className="whitespace-pre-wrap leading-relaxed text-gray-800">{activeText}</p>;
    }
    
    let highlightedContent = activeText;
  ...
  const isGenerating = isLoading || activeText === '生成を開始しています...';
  ...
  return (
    <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200 h-full min-h-[600px] flex flex-col">
      ...
      {/* 文章のみ再生成ボタン */}
      {!isGenerating && result.facilities && result.facilities.length > 0 && (
        <div className="mt-4 flex justify-center">
          <button
            onClick={onRegenerateText}
            className="text-sm font-bold text-orange-600 hover:text-white flex items-center bg-orange-50 hover:bg-orange-500 px-6 py-3 rounded-xl border border-orange-200 hover:border-orange-600 transition-all shadow-sm group"
          >
            <svg className="w-5 h-5 mr-2 text-orange-500 group-hover:text-white transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
