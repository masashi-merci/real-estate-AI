...
[VERSION_2]
【物件紹介文 バージョン2】
横浜の歴史とモダンが融合する「石川町」エリア。その中心に位置する「ブリシア横濱石川町」は、アクティブな都市生活を叶える絶好のロケーションにあります。
JR「石川町」駅から徒歩8分という近さは、通勤・通学のストレスを大幅に軽減。さらに地下鉄ブルーラインも利用でき、横浜の主要スポットへ自由自在にアクセスできます。
生活利便施設が徒歩圏内に揃う一方で、近隣には公園も多く、休日にはリフレッシュできる環境も魅力。単身者からファミリーまで、横浜での暮らしを存分に楽しみたい方におすすめの一邸です。
[/VERSION_2]

[VERSION_3]
【物件紹介文 バージョン3】
横浜市中区という価値あるアドレスに、洗練された都市生活の拠点が誕生しました。
「ブリシア横濱石川町」は、都会の利便性を享受しながらも、落ち着いた住環境を求める方のための上質なレジデンスです。
2駅2路線を使いこなす機動力は、ビジネスシーンでもプライベートでも大きなアドバンテージとなります。周辺には「まいばすけっと」などのスーパーやコンビニが充実し、日常の買い物に困ることはありません。
また、横浜公園などの緑豊かなスポットも身近にあり、都会の喧騒を忘れさせてくれる穏やかな時間も享受できます。洗練された外観デザインと、細部までこだわり抜かれた居住空間が、住まう方の誇りを満たします。
[/VERSION_3]

[FACILITIES_JSON]
[
  {"name": "石川町駅", "distance": "640m", "category": "駅"},
  {"name": "伊勢佐木長者町駅", "distance": "560m", "category": "駅"},
  {"name": "まいばすけっと", "distance": "200m", "category": "スーパー"},
  {"name": "ローソン", "distance": "150m", "category": "コンビニ"}
]
[/FACILITIES_JSON]`;
  ...
  for (const char of chunks) {
    current += char;
    onChunk(current.split('[FACILITIES_JSON]')[0]);
...
export const generatePropertyDescriptionStream = async (
  ...
  if (!apiKey) {
    ...
    return { fullText, texts: [fullText, fullText, fullText], groundingUrls: [], facilities: [] };
  }

  // 1. Google Maps APIを使用して正確な周辺情報を取得（既存データがあれば再利用）
  let nearbyFacilities: any[] = existingFacilities || [];
...
7. **3つのバリエーション作成**: 
   - 異なる切り口（例：1.ファミリー向け、2.利便性重視、3.洗練されたラグジュアリー・丁寧な文体など）で、3つの異なる物件紹介文を作成してください。
   - 各バージョンは [VERSION_1], [VERSION_2], [VERSION_3] というタグで囲んで出力してください。
...
[VERSION_2]
【物件紹介文 バージョン2】
(バージョン1とは異なる切り口で、400文字〜500文字程度で記述)
[/VERSION_2]

[VERSION_3]
【物件紹介文 バージョン3】
(さらに異なる切り口で、400文字〜500文字程度で記述)
[/VERSION_3]
...
指示:
1. 上記データを基に、正確で信頼性の高い紹介文を3つのバリエーションで作成してください。
  ...
  try {
    ...
    const responseStream = await ai.models.generateContentStream({
      ...
      config: {
        ...
        temperature: 0.5, // バリエーションを出すために少し上げる
    ...
    const texts: string[] = [];
    for (let i = 1; i <= 3; i++) {
      const match = fullText.match(new RegExp(`\\[VERSION_${i}\\]([\\s\\S]*?)\\[\\/VERSION_${i}\\]`));
      if (match && match[1]) {
    ...
    // 表示用テキストからJSONタグとバージョンタグを削除したものを基本テキストとする
    const finalDisplayText = fullText
      .replace(/\[VERSION_\d\][\s\S]*?\[\/VERSION_\d\]/g, '')
      .replace(/\[FACILITIES_JSON\][\s\S]*?\[\/FACILITIES_JSON\]/, '')
      .trim();
